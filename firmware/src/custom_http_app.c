...
HTTP_IO_RESULT TCPIP_HTTP_GetExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    int angle;
    char strAngleHorizontal[12];
    char strAngleVertical[12];
    ...
    if(!memcmp(filename, "index.htm", 9))
    {
        ...
        if(ptr && !memcmp(ptr, "getdistance", 11)) 
        {
            ...
            UARTinstance_PutString(1,"start ranging on #2");
        ...
        if(ptr) 
        {
            ...
            UARTinstance_PutString(1,strAngleHorizontal);
            //servo_setAngle(angle,2);
            UARTinstance_PutString(1,strAngleVertical);

        }
    }

    else if(!memcmp(filename, "webvend/lights.htm", 18))
    {
...
void TCPIP_HTTP_Print_angle(HTTP_CONN_HANDLE connHandle)
{
    ...
    itoa(angleString, servo_getAngle(), 10);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) angleString);
}
...
void TCPIP_HTTP_Print_distance(HTTP_CONN_HANDLE connHandle)
{
    ...
    itoa(distanceString, srf_getDistanceValue(), 10);            
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *) distanceString);
