...
char UARTinstance_PutChar(char uartInstance, char ch)
{
    if (uartInstance >= DRV_USART_INSTANCES_NUMBER) // maximum number of instances
        return -1;
    
    /*if(!(DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART1_TransferStatus()) )
    {
        DRV_USART1_WriteByte(ch);  // send modified byte
    }*/
    switch (uartInstance) {
        case 0:
            ( DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART0_TransferStatus() ) ?: DRV_USART0_WriteByte(ch);
#if DRV_USART_INSTANCES_NUMBER == 2
            break;
        case 1:
            ( DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART1_TransferStatus() ) ?: DRV_USART1_WriteByte(ch);
            break;
#elif DRV_USART_INSTANCES_NUMBER == 3
        case 2:
            ( DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART2_TransferStatus() ) ?: DRV_USART2_WriteByte(ch);
            break;
#elif DRV_USART_INSTANCES_NUMBER == 4
        case 3:
            ( DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART3_TransferStatus() ) ?: DRV_USART3_WriteByte(ch);
            break;
#elif DRV_USART_INSTANCES_NUMBER == 5
        case 4:
            ( DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART4_TransferStatus() ) ?: DRV_USART4_WriteByte(ch);
            break;
#endif
    }
...
char UARTinstance_PutString(char uartInstance, char szData[])
{
    char *pData = szData;
    ...
    while(*pData)
    {
        UARTinstance_PutChar(uartInstance, (*(pData++)));
    }
...
char UARTinstance_AvalaibleRx(char uartInstance)
{
    if (uartInstance >= DRV_USART_INSTANCES_NUMBER) // maximum number of instances
        return -1;
    ...
    switch (uartInstance) {
        case 0:
            return DRV_USART0_ReceiverBufferIsEmpty() ? false : true;
#if DRV_USART_INSTANCES_NUMBER == 2
        case 1:
            return DRV_USART1_ReceiverBufferIsEmpty() ? false : true;
#elif DRV_USART_INSTANCES_NUMBER == 3
        case 2:
            return DRV_USART2_ReceiverBufferIsEmpty() ? false : true;
#elif DRV_USART_INSTANCES_NUMBER == 4
        case 3:
            return DRV_USART3_ReceiverBufferIsEmpty() ? false : true;
#elif DRV_USART_INSTANCES_NUMBER == 5
        case 4:
            return DRV_USART4_ReceiverBufferIsEmpty() ? false : true;
#endif
        default:
            return 0;
    }
...
{
    while (!UART_AvailableRx());
    return U4RXREG;
}

char UARTinstance_GetCharPoll(char uartInstance) 
{
    if (uartInstance >= DRV_USART_INSTANCES_NUMBER) // maximum number of instances
        return -1;

    //while (!UARTinstance_AvailableRx());
    switch (uartInstance) {
        case 0:
            return (unsigned char) DRV_USART0_ReadByte();
#if DRV_USART_INSTANCES_NUMBER == 2
        case 1:
            return (unsigned char) DRV_USART1_ReadByte();
#elif DRV_USART_INSTANCES_NUMBER == 3
        case 2:
            return (unsigned char) DRV_USART2_ReadByte();
#elif DRV_USART_INSTANCES_NUMBER == 4
        case 3:
            return (unsigned char) DRV_USART3_ReadByte();
#elif DRV_USART_INSTANCES_NUMBER == 5
        case 4:
            return (unsigned char) DRV_USART4_ReadByte();
#endif
        default:
            return 0;
    }
...
*/
unsigned char UART_GetStringPoll(char *pText)
{
    int idx = 0;
...
char UARTinstance_GetStringPoll(char uartInstance, char *pText)
{
    int idx = 0;
    ...
    while(UARTinstance_AvalaibleRx(uartInstance))
    {
        /*if ((uartInstance == 1) && (idx == 0))
            UARTinstance_PutString(0,"^");*/
        pText[idx++] = UARTinstance_GetCharPoll(uartInstance);
        if ((uartInstance == 1) && (idx > 0))
        {
            if(pText[idx-1] == '\r')
                UARTinstance_PutString(0,"CR");
            else if(pText[idx-1] == '\n')
                UARTinstance_PutString(0,"LF");
            else if(pText[idx-1] == '#')
                UARTinstance_PutString(0,"HASH");
            /*else
                UARTinstance_PutChar(0,pText[idx-1]);*/
        }
    }
    if(idx != 0)
    {
...
char UARTinstance_GetString( char uartInstance, char* pchBuff, int cchBuff )
{
	unsigned char ich;
